...
describe('OPLAccountAbstraction', function () {
  describe('Integration', function () {
    it('Should work with OPL secret deposit', async function () {
      ...
      const OPLAccountAbstractionEnclave = await ethers.getContractFactory(
      ...
      const oplAccountAbstractionEnclave =
        await OPLAccountAbstractionEnclave.deploy(expectedPaymasterAddress);
      await oplAccountAbstractionEnclave.deployed();
      const OPLAccountAbstractionPaymaster = await ethers.getContractFactory(
      ...
      const sendSecretGasFeeToHostPaymasterTx =
        await oplAccountAbstractionEnclave.sendSecretGasFeeToHostPaymaster(
          accountAddress,
          userOpHash,
          { value: ethers.utils.parseEther('0.1') },
        );
      await expect(sendSecretGasFeeToHostPaymasterTx).to.emit(
        oplAccountAbstractionPaymaster,
        'SendSecretGasFee',
      );
