...
contract ExtendedOPLBaseEndpoint is Context {
    ...
    constructor(address _remote, uint256 _remoteChainId, address _messageBus, bool _inOrder) {
        ...
        inOrder = _inOrder;
        if (!_isLocalNetwork() && _remoteChainId == block.chainid) revert SelfCallDisallowed();
    }
    ...
    function postMessage(bytes memory _method, bytes memory _message) internal returns (uint256) {
        bytes memory envelope = abi.encodePacked(bytes4(keccak256(_method)), txSeq, _message);
        uint256 fee = estimateFee(envelope.length);
        if (_isLocalNetwork()) {
            uint256 celerStatus = ExtendedOPLBaseEndpoint(messageBus).executeMessage(
                address(this), // sender
            ...
            ICelerMessageBus(messageBus).sendMessage{value: fee}(remote, remoteChainId, envelope);
        }
        ++txSeq;
        return fee;
    }
    ...
    function postMessageWithTransfer(
        ...
        uint256 fee = estimateFee(envelope.length);
        if (_isLocalNetwork()) {
            // not required bridge and transferId in local execution
            IERC20(_token).transferFrom(msg.sender, messageBus, _amount);
            uint256 celerStatus = ExtendedOPLBaseEndpoint(messageBus).executeMessageWithTransfer(
                address(this), // sender
                _token,
                _amount,
                uint64(block.chainid),
            ...
            // Only using liquidityBridge for now
            ...
            bytes32 transferId = _computeLiqBridgeTransferId(
                remote,
                _token,
                _amount,
                uint64(remoteChainId),
            ...
            ICelerMessageBus(messageBus).sendMessageWithTransfer{value: fee}(
                remote,
                remoteChainId,
                bridge,
                transferId,
    ...
    function executeMessage(
        ...
        if (_sender != remote || _senderChainId != remoteChainId) revert NotRemoteEndpoint();
    ...
    // Extended
    // Copied from
    // https://github.com/celer-network/sgn-v2-contracts/blob/main/contracts/message/libraries/MessageSenderLib.sol
    function _computeLiqBridgeTransferId(
        address _receiver,
        address _token,
        uint256 _amount,
        uint64 _dstChainId,
 ...
 */
contract ExtendedOPLEndpoint is ExtendedOPLBaseEndpoint {
    constructor(
        address _remote,
        bytes32 _remoteChainName
    )
        ExtendedOPLBaseEndpoint(
            _remote,
